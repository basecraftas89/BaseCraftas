...
test('browser sanitizer blocks DOM XSS and unsafe paste markup',()=>{
 const dom=new JSDOM('<div id="body"></div>',{url:'https://basecraftas.com/apps/tsuzuri-studio/',runScripts:'dangerously'});
 dom.window.eval(readFileSync('apps/tsuzuri-studio/security.js','utf8'));
...
test('draft media private; public snapshot required; unpublish revokes it; existing SVG denied',async()=>{
 ...
 assert.equal((await f.call('/api/tsuzuri-studio/media/'+asset.key,'GET',undefined,'')).status,403);
 const privateResponse=await f.call('/api/tsuzuri-studio/media/'+asset.key);assert.equal(privateResponse.status,200);assert.match(privateResponse.headers.get('cache-control'),/no-store/);assert.equal(privateResponse.headers.get('x-content-type-options'),'nosniff');assert.match(privateResponse.headers.get('content-security-policy'),/sandbox/);
...
test('cross-origin/missing-origin requests blocked and body type enforced',()=>{
 const url='https://basecraftas.com/api/tsuzuri-studio/api/articles';
...
test('weekend event publication requires only a thumbnail',async()=>{
 const f=fixture();
 const article=await f.article({content_type:'weekend'});
...
test('production UI waits for authentication; pending drafts use scoped session storage and logout clears views',async()=>{
 const dom=new JSDOM(readFileSync('apps/tsuzuri-studio/index.html','utf8'),{url:'https://basecraftas.com/apps/tsuzuri-studio/',runScripts:'outside-only'}),w=dom.window;
 w.scrollTo=()=>{};w.HTMLDialogElement.prototype.close=function(){this.open=false};
 w.eval(readFileSync('apps/tsuzuri-studio/security.js','utf8'));
 ...
 w.eval(readFileSync('apps/tsuzuri-studio/script.js','utf8').replace(/\}\)\(\);\s*$/,'window.securityTest={state,persist,clearSessionView};})();'));
 ...
 const cached=JSON.parse(w.sessionStorage.getItem('tsuzuri-studio-session:admin@example.com'));
