...
export function safeUrl(value, relative = false) {
  ...
  try {
    ...
    if (url.hostname === 'basecraftas.com' && /^\/api\/(?:tsuzuri|column)-studio\/media\//.test(url.pathname)) {
      return 'https://basecraftas.com/column-media/' + url.pathname.replace(/^\/api\/(?:tsuzuri|column)-studio\/media\//, '');
    }
    return raw;
  } catch { return ''; }
}
...
export function requestGuard(request, env, path) {
  ...
  const multipart = path === '/api/assets';
