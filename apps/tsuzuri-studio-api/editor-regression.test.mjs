...
w.eval(js.replace(/\}\)\(\);\s*$/, 'window.editorTest={bootRemote,uploadHero,state,els,blankPost,collect,editPost,scheduleSave,syncRemoteArticle,sourceFromUrl,analyzeLink,applyLinkPreview,setHeroPreview,showHistory,publish,insertBubble,insertMemberBoundary,openTextLinkDialog,applyTextLink,removeTextLink,checkpoint,api,openLatest,changeLifecycle,openContentView,setCloud:(value)=>{cloudReady=value;},setApi:(value)=>{API_BASE=value;}};})();'));
...
assert.match(w.document.querySelector('[data-create-type="video"]').textContent,/YouTube動画をつまみに追加/);
...
assert.equal(w.document.getElementById('contentCreateButton').textContent,'＋ YouTube動画を追加');
assert.match(w.document.getElementById('contentListCopy').textContent,/動画カードとして直接公開/);
...
assert.equal(w.document.getElementById('linkIntakeTitle').textContent,'YouTubeリンクからつまみに追加');
assert.equal(w.document.querySelector('[data-action="publish"]').textContent,'つまみに公開する');
...
t.els.mediaUrl.value='https://youtu.be/BMhHEeAj6-I';t.scheduleSave();assert.equal(t.els.title.value,'無題のコンテンツ');
t.applyLinkPreview({recommended_content_type:'video',kind:'video',source_id:'BMhHEeAj6-I',title:'AIで生成したものはバレる!? 文章への見えない透かし記号',description:'',image:'https://i.ytimg.com/vi/BMhHEeAj6-I/hqdefault.jpg'});
assert.equal(t.els.title.value,'AIで生成したものはバレる!? 文章への見えない透かし記号');
t.els.title.value='運営が修正したタイトル';t.applyLinkPreview({recommended_content_type:'video',kind:'video',source_id:'BMhHEeAj6-I',title:'再取得タイトル'});assert.equal(t.els.title.value,'運営が修正したタイトル');
...
assert.equal(w.document.getElementById('textLinkDialog').open,true);assert.match(w.document.getElementById('textLinkSelection').textContent,/リンク/);
w.document.getElementById('textLinkUrl').value='https://example.com/source';t.applyTextLink();
const textLink=t.els.editor.querySelector('a');assert.equal(textLink.textContent,'リンク');assert.equal(textLink.getAttribute('href'),'https://example.com/source');assert.equal(textLink.getAttribute('target'),'_blank');
const removeRange=w.document.createRange();removeRange.selectNodeContents(textLink);linkSelection.removeAllRanges();linkSelection.addRange(removeRange);t.openTextLinkDialog();t.removeTextLink();assert.equal(t.els.editor.querySelector('a'),null);assert.match(t.els.editor.textContent,/リンク/);
