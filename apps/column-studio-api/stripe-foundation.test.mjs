...
function stripeFixture(){
  ...
  sql.exec(readFileSync('apps/column-studio-api/migrations/20260918_customer_email_auth.sql','utf8'));
  ...
  const env={STRIPE_MODE:'test',STRIPE_WEBHOOK_SECRET:secret,DB:{prepare,async batch(items){sql.exec('BEGIN');try{const results=[];for(const item of items)results.push(await item.run());sql.exec('COMMIT');return results;}catch(error){sql.exec('ROLLBACK');throw error;}}}};
...
test('認証済み会員だけが自分のStripe Customer Portalを作成できる',async()=>{
  ...
  try{
    ...
    assert.equal(portalParams.get('return_url'),'https://basecraftas.com/projects/totonoe/curriculum/mypage.html?billing=returned');
...
test('会員画面は再ログインと契約管理の導線を備える',()=>{
  const weekly=readFileSync('projects/totonoe/weekly/index.html','utf8');
  const login=readFileSync('projects/totonoe/weekly/login.html','utf8');
  const loginScript=readFileSync('projects/totonoe/weekly/login.js','utf8');
  const mypage=readFileSync('projects/totonoe/curriculum/mypage.html','utf8');
  const mypageScript=readFileSync('projects/totonoe/curriculum/mypage.js','utf8');
