...
test('資格画像は非公開R2に保存され、管理者審査後だけセラピスト認証になる',async()=>{
  ...
  const listed=await worker.fetch(new Request('https://test.local/api/column-studio/api/admin/qualifications',{headers:adminHeaders}),fixture.env);
  ...
  const image=await worker.fetch(new Request(`https://test.local/api/column-studio/api/admin/qualifications/${submittedBody.submission.id}/image`,{headers:adminHeaders}),fixture.env);
  ...
  try{
    ...
    reviewed=await worker.fetch(new Request(`https://test.local/api/column-studio/api/admin/qualifications/${submittedBody.submission.id}`,{method:'PATCH',headers:{...adminHeaders,'content-type':'application/json'},body:JSON.stringify({status:'verified',review_note:'確認済み'})}),fixture.env);
