(function(){
  ...
  var lastAnalyzedUrl='';
  var billingLoaded=false,billingLoading=false;
...
state.members=[memberFromApi(me.member)];
  ...
  document.getElementById('billingMonth').value=japanMonth();
  renderTopicTagSuggestions();syncDestinationFields();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();var initialView=location.hash.replace('#','');if(['tsuzuri','column','podcast','video','archive','weekend'].includes(initialView))openContentView(initialView);bootRemote();
})();
