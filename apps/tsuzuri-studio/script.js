...
state.members=[memberFromApi(me.member)];
  ...
  function renderWaitlist(data){
    ...
    document.getElementById('enrollmentCapacity').textContent='受付枠 '+Number(enrollment.capacity||0)+'名 ／ 利用中・予約中 '+Number(enrollment.used||0)+'名 ／ 残り '+Number(enrollment.remaining||0)+'名'+(enrollment.full?' （満枠）':'');
  ...
  async function increaseCapacity(button){
    if(state.role!=='admin'||button.disabled)return;
    if(!window.confirm('TAYORIの受付枠を10名追加しますか？'))return;
    button.disabled=true;
    try{var enrollment=await api('/api/admin/plan-capacity/increase',{method:'POST',body:JSON.stringify({plan_code:'weekly_monthly'})});var current=await api('/api/admin/waitlist');current.enrollment=enrollment;renderWaitlist(current);toast('TAYORIの受付枠を10名追加しました');}
    catch(error){toast(error.message||'受付枠を追加できませんでした');}
    finally{button.disabled=false;}
  }
