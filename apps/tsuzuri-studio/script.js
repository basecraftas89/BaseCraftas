(function(){
  ...
  var API_BASE=window.TOTONOE_STUDIO_API_BASE||window.TSUZURI_STUDIO_API_BASE||window.COLUMN_STUDIO_API_BASE||(location.hostname==='basecraftas.com'?'/api/totonoe-studio':'');
  var now=new Date();
  ...
  var CONTENT_CHARACTERS=[{id:'mion',name:'ミオン'},{id:'tsugumo',name:'ツグモ'},{id:'hakuto',name:'ハクト'},{id:'mion-tsugumo-hakuto',name:'ミオン・ツグモ・ハクト'}];
  var CONTENT_TYPES={column:'つづり｜TSUZURI',podcast:'Podcast',video:'動画',archive:'アーカイブ',seminar:'セミナー',learning:'学習',weekend:'次回開催'};
  var CONTENT_TARGETS={column:'つづり｜TSUZURI',podcast:'Podcast',video:'つまみ｜TSUMAMI',archive:'アーカイブ動画',seminar:'セミナー',learning:'つまみ｜TSUMAMI',weekend:'週末のAI整え習慣「次回の開催について」'};
  ...
  var lastAnalyzedUrl='';
  var billingLoaded=false,billingLoading=false,analyticsLoaded=false,analyticsLoading=false,analyticsCurrentData=null;
  ...
  function personById(id){return TOTONOE_MEMBERS.concat(CONTENT_CHARACTERS).find(function(person){return person.id===id;})||TOTONOE_MEMBERS[0];}
  ...
  function clearSessionView(){
    ...
    sessionKey='';sessionExpires=0;state.currentEmail='';state.posts=[];state.members=[];state.archiveCandidates=[];state.role='viewer';state.editingId=null;
    els.editor.replaceChildren();els.title.value='';els.excerpt.value='';setHeroPreview('');
    document.getElementById('previewBody').replaceChildren();var previewHero=document.getElementById('previewHero');previewHero.replaceChildren();previewHero.hidden=true;document.getElementById('historyDetail').textContent='';document.getElementById('historyList').replaceChildren();
    ...
    applyRole('viewer');renderDashboard();renderPosts();renderMembers();
  }
  function logout(){
  ...
  function isSourceOnlyType(type){return ['podcast','archive','video','learning','weekend'].includes(normalizeType(type));}
  function syncEditorMode(){
    ...
    var copy=type==='archive'?['Google DriveのアーカイブURLを貼り付け','第何回・開催日・タイトルを確認して公開します。','https://drive.google.com/file/d/.../view','アーカイブのカテゴリタグ']:type==='podcast'?['stand.fmのURLを貼り付け','stand.fmのリンクからタイトルと概要を読み込みます。','https://stand.fm/episodes/...','Podcastのカテゴリタグ']:['YouTubeリンクからつまみに追加','リンクからタイトルとサムネイルを取得し、つまみへ直接公開します。','https://www.youtube.com/watch?v=...','動画のカテゴリタグ'];
    ...
    document.getElementById('sourceDetailFields').classList.toggle('source-only-hidden',type==='weekend');
    document.querySelector('.topic-tag-field').classList.toggle('source-only-hidden',type==='weekend');
    ...
    document.querySelector('[data-action="publish"]').textContent=type==='video'?'つまみに公開する':'公開する';
  ...
  function syncDestinationFields(){var isColumn=normalizeType(els.contentType.value)==='column';if(!isColumn){els.destination.value='tsuzuri';els.category.value=els.category.value==='character-story'?'content':els.category.value;}else if(els.destination.value==='characters'){els.category.value='character-story';}else if(els.category.value==='character-story'){els.category.value='content';}els.destination.disabled=!isColumn;}
  function syncSourceFields(){var type=normalizeType(els.contentType.value);document.getElementById('sourceDetailFields').hidden=['podcast','archive','seminar','weekend'].indexOf(type)===-1;syncEditorMode();}
  ...
  function applyLinkPreview(preview){
    ...
    syncDestinationFields();syncSourceFields();scheduleSave();
  }
...
state.members=[memberFromApi(me.member)];
      ...
      cloudReady=true;
      persist();applyRole(state.role);renderDashboard();renderPosts();renderMembers();
      if(document.getElementById('view-billing').classList.contains('active')){billingLoaded=false;loadBilling();}
      if(document.getElementById('view-analytics').classList.contains('active')){analyticsLoaded=false;loadAnalytics();}
  ...
  function showView(name){if(busyPublishing)return;flushPending();if(name==='editor'&&state.role==='viewer'){toast('閲覧者は記事を編集できません');return;}if((name==='members'||name==='billing'||name==='settings')&&state.role!=='admin'){toast('この画面は管理者専用です。管理者アカウントで再ログインしてください。');return;}document.querySelectorAll('.view').forEach(function(v){v.classList.toggle('active',v.id==='view-'+name);});document.querySelectorAll('.nav-item').forEach(function(b){var active=name==='posts'?b.dataset.contentView===currentTypeFilter:b.dataset.view===name;b.classList.toggle('active',active);});var active=document.getElementById('view-'+name);document.getElementById('breadcrumb').innerHTML='<span>ToToNoE+ Studio</span><b>/</b> '+esc(name==='editor'?contentTypeLabel(els.contentType.value)+'を編集':name==='posts'?(currentTypeFilter==='all'?'すべてのコンテンツ':contentTypeLabel(currentTypeFilter)):active.dataset.title);if(name==='posts')renderPosts();if(name==='dashboard')renderDashboard();if(name==='members'){renderMembers();if(API_BASE)api('/api/members').then(function(data){state.members=(data.members||[]).map(memberFromApi);renderMembers();}).catch(function(error){toast(error.message);});}if(name==='billing'&&!billingLoaded)loadBilling();if(name==='analytics'&&!analyticsLoaded)loadAnalytics();document.getElementById('sidebar').classList.remove('open');window.scrollTo(0,0);}
  ...
  function shortMonth(value){var parts=String(value||'').split('-');return parts.length===2?Number(parts[1])+'月':value;}
  function setBillingLoading(message){document.getElementById('billingQuality').textContent=message;document.getElementById('billingMonthlyChart').innerHTML='<div class="empty-state">'+esc(message)+'</div>';document.getElementById('billingComposition').innerHTML='<div class="empty-state">'+esc(message)+'</div>';}
  function renderBillingChart(monthly){
    var host=document.getElementById('billingMonthlyChart');
    if(!monthly.length){host.innerHTML='<div class="empty-state">表示できる月別データがありません。</div>';return;}
    var width=900,height=300,left=48,right=38,top=24,base=228,bottom=42,plotWidth=width-left-right;
    var maxPositive=Math.max(1,...monthly.map(function(item){return Math.max(0,Number(item.recurring_yen||0)+Number(item.entry_fee_yen||0)+Number(item.adjustment_yen||0));}));
    var maxRefund=Math.max(1,...monthly.map(function(item){return Math.abs(Math.min(0,Number(item.refund_yen||0)));}));
    var maxMembers=Math.max(1,...monthly.map(function(item){return Number(item.month_end_active_customers||0);}));
    var step=plotWidth/monthly.length,barWidth=Math.min(34,step*.52),parts=[];
    parts.push('<svg viewBox="0 0 '+width+' '+height+'" role="img" aria-label="月別決済額と月末有効会員数"><line x1="'+left+'" y1="'+base+'" x2="'+(width-right)+'" y2="'+base+'" class="billing-axis"/>');
    [0,.5,1].forEach(function(rate){var y=base-rate*174;parts.push('<line x1="'+left+'" y1="'+y+'" x2="'+(width-right)+'" y2="'+y+'" class="billing-gridline"/><text x="'+(left-7)+'" y="'+(y+4)+'" text-anchor="end" class="billing-axis-label">'+esc(yen(maxPositive*rate))+'</text>');});
    var linePoints=[];
    monthly.forEach(function(item,index){
      var x=left+step*index+step/2,y=base;
      var segments=[['recurring_yen','billing-bar-recurring'],['entry_fee_yen','billing-bar-entry'],['adjustment_yen','billing-bar-adjustment']];
      segments.forEach(function(segment){var amount=Math.max(0,Number(item[segment[0]]||0));if(!amount)return;var h=amount/maxPositive*174;y-=h;parts.push('<rect x="'+(x-barWidth/2)+'" y="'+y+'" width="'+barWidth+'" height="'+h+'" class="'+segment[1]+'"/>');});
      var refund=Math.abs(Math.min(0,Number(item.refund_yen||0)));if(refund)parts.push('<rect x="'+(x-barWidth/2)+'" y="'+base+'" width="'+barWidth+'" height="'+(refund/maxRefund*24)+'" class="billing-bar-refund"/>');
      parts.push('<text x="'+x+'" y="'+(height-bottom+20)+'" text-anchor="middle" class="billing-axis-label">'+esc(shortMonth(item.month))+'</text>');
      parts.push('<title>'+esc(item.month+' 決済額 '+yen(item.net_yen)+(item.month_end_active_customers===null?' / 会員数 履歴なし':' / 会員 '+item.month_end_active_customers+'人'))+'</title>');
      if(item.month_end_active_customers!==null){var ly=base-Number(item.month_end_active_customers)/maxMembers*174;linePoints.push(x+','+ly);parts.push('<circle cx="'+x+'" cy="'+ly+'" r="4" class="billing-member-point"><title>'+esc(item.month+' '+item.month_end_active_customers+'人')+'</title></circle>');}
    });
    if(linePoints.length>1)parts.push('<polyline points="'+linePoints.join(' ')+'" class="billing-member-line"/>');
    parts.push('<text x="'+(width-right)+'" y="18" text-anchor="end" class="billing-axis-label">会員数 最大 '+maxMembers+'人</text></svg>');
    host.innerHTML=parts.join('');
  }
  function renderBillingComposition(breakdown){
    var totals={weekly:{label:'TAYORI',amount:0,people:0},curriculum:{label:'IROHA',amount:0,people:0}};
    breakdown.forEach(function(row){var target=totals[row.product_code];if(!target)return;target.amount+=Number(row.recurring_monthly_yen||0);target.people+=Number(row.active_customers||0)+Number(row.trialing_customers||0);});
    var list=[totals.weekly,totals.curriculum],max=Math.max(1,...list.map(function(item){return item.amount;}));
    if(!list.some(function(item){return item.amount>0;})){document.getElementById('billingComposition').innerHTML='<div class="empty-state">継続課金中の契約はありません。</div>';return;}
    document.getElementById('billingComposition').innerHTML=list.map(function(item){return '<div class="composition-row"><div><strong>'+item.label+'</strong><span>'+item.people+'人</span></div><div class="composition-track"><i style="width:'+Math.max(item.amount/max*100,item.amount?3:0)+'%"></i></div><b>'+yen(item.amount)+'/月</b></div>';}).join('');
  }
  function renderBilling(data){
    ...
    document.getElementById('billingMonthNet').textContent=quality.has_transaction_history?yen(kpis.selected_month_net_yen):'—';
    document.getElementById('billingMonthDetail').textContent=quality.has_transaction_history?(quality.current_month_partial?'月初から現在までの途中経過':'支払済み − 返金'):'決済履歴はまだありません';
    document.getElementById('billingRecurring').textContent=yen(kpis.recurring_monthly_yen)+'/月';
    document.getElementById('billingRecurringDetail').textContent='現在の有効契約を月額換算';
    var qualityMessages=[];if(Number(quality.failed_webhook_count||0)>0)qualityMessages.push('Webhook失敗 '+quality.failed_webhook_count+'件');if(Number(kpis.attention_customers||0)>0)qualityMessages.push('支払い要確認 '+kpis.attention_customers+'人');if(!quality.has_subscription_history)qualityMessages.push('過去の会員推移は記録開始後から表示');
    document.getElementById('billingQuality').textContent=qualityMessages.length?'要確認：'+qualityMessages.join(' ／ '):'課金データは正常に読み込まれています。';
    document.getElementById('billingQuality').classList.toggle('warning',qualityMessages.length>0);
    renderBillingChart(data.monthly||[]);renderBillingComposition(data.breakdown||[]);
    document.getElementById('billingBreakdown').innerHTML=(data.breakdown||[]).map(function(row){return '<tr><th scope="row">'+esc(row.label)+'</th><td>'+Number(row.active_customers||0)+'人</td><td>'+Number(row.trialing_customers||0)+'人</td><td>'+Number(row.cancel_scheduled_customers||0)+'人</td><td class="'+(row.attention_customers?'attention':'')+'">'+Number(row.attention_customers||0)+'人</td><td>'+yen(row.recurring_monthly_yen)+'</td></tr>';}).join('')||'<tr><td colspan="6">該当する契約はありません。</td></tr>';
  }
  ...
  async function loadBilling(){
    ...
    if(!API_BASE||!cloudReady){setBillingLoading('共有環境へ接続すると課金情報を確認できます。');return;}
    billingLoading=true;setBillingLoading('課金情報を読み込んでいます。');
    var params=new URLSearchParams({month:document.getElementById('billingMonth').value||japanMonth(),range:document.getElementById('billingRange').value,product:document.getElementById('billingProduct').value,audience:'all'});
    try{var results=await Promise.all([api('/api/admin/billing-summary?'+params.toString()),api('/api/admin/waitlist')]);renderBilling(results[0]);renderWaitlist(results[1]);billingLoaded=true;}
    catch(error){setBillingLoading(error.data&&error.data.error==='billing_dashboard_not_configured'?'課金集計用のデータベース更新が必要です。':error.message);toast(error.message);}
    finally{billingLoading=false;}
  }
  var analyticsSample={generated_at:'2026-09-23T08:48:39Z',preview:true,filters:{start:'2026-09-16',end:'2026-09-23'},kpis:{visits:71,identified_visits:3,direct_visits:68,ai_visits:0},comparison:{start:'2026-09-08',end:'2026-09-15',kpis:{visits:58,identified_visits:2,direct_visits:56,ai_visits:0},sources:[{label:'直接・判別不能',visits:56},{label:'YouTube',visits:2}]},sources:[{label:'直接・判別不能',visits:68},{label:'Prairie Card',visits:2},{label:'YouTube',visits:1}],landings:[{label:'ToToNoE+ トップ',path:'/projects/totonoe/',visits:17},{label:'つまみ｜TSUMAMI',path:'/projects/totonoe/tsumami/',visits:13},{label:'運営チーム',path:'/projects/totonoe/team',visits:13},{label:'たより｜TAYORI',path:'/projects/totonoe/tayori',visits:8},{label:'コンテンツ',path:'/projects/totonoe/contents',visits:5},{label:'つづり｜TSUZURI',path:'/projects/totonoe/tsuzuri/',visits:5}],daily:[{date:'2026-09-16',visits:8},{date:'2026-09-17',visits:6},{date:'2026-09-18',visits:7},{date:'2026-09-19',visits:10},{date:'2026-09-20',visits:9},{date:'2026-09-21',visits:11},{date:'2026-09-22',visits:12},{date:'2026-09-23',visits:8}],entries:[{date:'2026-09-23',source:'直接・判別不能',referer_host:'',landing_page:'ToToNoE+ トップ',request_path:'/projects/totonoe/',visits:7},{date:'2026-09-23',source:'YouTube',referer_host:'youtube.com',landing_page:'つまみ｜TSUMAMI',request_path:'/projects/totonoe/tsumami/',visits:1},{date:'2026-09-22',source:'直接・判別不能',referer_host:'',landing_page:'運営チーム',request_path:'/projects/totonoe/team',visits:10},{date:'2026-09-22',source:'Prairie Card',referer_host:'my.prairie.cards',landing_page:'ToToNoE+ トップ',request_path:'/projects/totonoe/',visits:2},{date:'2026-09-21',source:'直接・判別不能',referer_host:'',landing_page:'たより｜TAYORI',request_path:'/projects/totonoe/tayori',visits:11}],journeys:[{from_label:'コンテンツ',to_label:'ToToNoE+ トップ',count:5},{from_label:'Base Craftas プロジェクト一覧',to_label:'ToToNoE+ トップ',count:5},{from_label:'ToToNoE+ トップ',to_label:'たより｜TAYORI',count:4},{from_label:'ToToNoE+ トップ',to_label:'キャラクター',count:4},{from_label:'ToToNoE+ トップ',to_label:'サービス',count:4},{from_label:'サービス',to_label:'コンテンツ',count:4},{from_label:'サービス',to_label:'たより｜TAYORI',count:3},{from_label:'コンテンツ',to_label:'つまみ｜TSUMAMI',count:3}]};
  analyticsSample.initiatives=[{id:'preview-initiative',occurred_on:'2026-09-19',title:'XでTAYORI紹介投稿',channel:'x',initiative_type:'social_post',destination_path:'/projects/totonoe/tayori',objective:'TAYORIの認知とサービス閲覧を増やす',hypothesis:'具体的な利用場面を伝えるとTAYORIへの移動が増える',owner:'小島 健',primary_owner_id:'kojima-ken',collaborator_ids:['shindo-toshiki'],reference_url:'',notes:'ローカル画面確認用のサンプル施策',result_status:'partial',learning:'具体例の投稿は反応があった',next_action:'次回は誘導先を本文にも明記する',review_on:'2026-09-26'}];
  function analyticsDate(daysAgo){var date=new Date(Date.now()+9*60*60*1000-daysAgo*86400000);return date.toISOString().slice(0,10);}
  function syncAnalyticsDates(){var range=document.getElementById('analyticsRange').value,custom=range==='custom',start=document.getElementById('analyticsStart'),end=document.getElementById('analyticsEnd');start.disabled=!custom;end.disabled=!custom;if(!custom){end.value=analyticsDate(0);start.value=analyticsDate(Number(range)-1);}}
  function setAnalyticsLoading(message,warning){analyticsCurrentData=null;var quality=document.getElementById('analyticsQuality');quality.textContent=message;quality.classList.toggle('warning',Boolean(warning));document.getElementById('analyticsInsight').textContent='選択期間の変化を集計しています。';['analyticsSources','analyticsLandings','analyticsTrend','analyticsHeatmap','analyticsInitiatives','analyticsJourneys'].forEach(function(id){document.getElementById(id).innerHTML='<div class="analytics-empty">'+esc(message)+'</div>';});document.getElementById('analyticsDailySources').innerHTML='<tr><td colspan="5">'+esc(message)+'</td></tr>';document.querySelector('[data-action="export-analytics-csv"]').disabled=true;document.querySelector('[data-action="export-analytics-initiatives"]').disabled=true;}
  function renderAnalyticsBars(id,items){var host=document.getElementById(id),list=(items||[]).slice(0,10),max=Math.max(1,...list.map(function(item){return Number(item.visits||0);}));host.innerHTML=list.length?list.map(function(item){var value=Number(item.visits||0);return '<div class="analytics-bar-row" title="'+esc((item.path?item.path+' / ':'')+value+'件')+'"><span class="analytics-bar-label">'+esc(item.label)+'</span><span class="analytics-bar-track"><i style="width:'+Math.max(2,value/max*100)+'%"></i></span><strong class="analytics-bar-value">'+value.toLocaleString('ja-JP')+'</strong></div>';}).join(''):'<div class="analytics-empty">該当する流入はありません。</div>';}
  function renderAnalyticsTrend(items,initiatives){var host=document.getElementById('analyticsTrend'),list=items||[],byDate=new Map;for(var initiative of initiatives||[]){var date=initiative.occurred_on,dates=byDate.get(date)||[];dates.push(initiative);byDate.set(date,dates);}var max=Math.max(1,...list.map(function(item){return Number(item.visits||0);}));host.innerHTML=list.length?list.map(function(item,index){var value=Number(item.visits||0),show=list.length<=14||index%Math.ceil(list.length/12)===0,markers=byDate.get(item.date)||[],marker=markers.length?'<button type="button" class="analytics-trend-marker" data-edit-initiative="'+esc(markers[0].id)+'" title="'+esc(markers.map(function(entry){return entry.title;}).join('／'))+'">'+markers.length+'</button>':'';return '<div class="analytics-trend-item'+(markers.length?' has-initiative':'')+'" title="'+esc(item.date+' '+value+'件')+'">'+marker+'<i class="analytics-trend-bar" style="height:'+Math.max(2,value/max*100)+'%"></i><small>'+esc(show?item.date.slice(5).replace('-','/'):'')+'</small></div>';}).join(''):'<div class="analytics-empty">期間内の推移データはありません。</div>';}
  function renderAnalyticsJourneys(items){var host=document.getElementById('analyticsJourneys'),list=(items||[]).slice(0,15);host.innerHTML=list.length?list.map(function(item){var from=item.from_path||item.from_label,to=item.to_path||item.to_label;return '<div class="analytics-journey" title="'+esc(from+' → '+to)+'"><span>'+esc(item.from_label)+'</span><b aria-hidden="true">→</b><span>'+esc(item.to_label)+'</span><strong>'+Number(item.count||0).toLocaleString('ja-JP')+'回</strong></div>';}).join(''):'<div class="analytics-empty">期間内に確認できるページ間移動はありません。</div>';}
  function renderAnalyticsDailySources(items){var rows=(items||[]).slice().sort(function(a,b){return b.date.localeCompare(a.date)||Number(b.visits||0)-Number(a.visits||0)||a.source.localeCompare(b.source,'ja');});document.getElementById('analyticsDailySources').innerHTML=rows.length?rows.map(function(row){return '<tr><td>'+esc(row.date)+'</td><td><span class="analytics-source-badge">'+esc(row.source)+'</span></td><td>'+esc(row.referer_host||'—')+'</td><td title="'+esc(row.request_path||'')+'">'+esc(row.landing_page)+'</td><td>'+Number(row.visits||0).toLocaleString('ja-JP')+'</td></tr>';}).join(''):'<tr><td colspan="5">期間内の流入データはありません。</td></tr>';}
  function renderAnalyticsHeatmap(items,daily){var entries=items||[],dates=(daily||[]).map(function(item){return item.date;}),sourceTotals=new Map;entries.forEach(function(row){sourceTotals.set(row.source,(sourceTotals.get(row.source)||0)+Number(row.visits||0));});var sources=[...sourceTotals].sort(function(a,b){return b[1]-a[1];}).slice(0,8).map(function(item){return item[0];}),values=new Map,max=1;entries.forEach(function(row){var key=row.date+'|'+row.source,value=(values.get(key)||0)+Number(row.visits||0);values.set(key,value);max=Math.max(max,value);});var host=document.getElementById('analyticsHeatmap');if(!dates.length||!sources.length){host.innerHTML='<div class="analytics-empty">期間内の媒体別データはありません。</div>';return;}var html='<div class="analytics-heatmap-head"><span>日付</span>'+sources.map(function(source){return '<strong title="'+esc(source)+'">'+esc(source)+'</strong>';}).join('')+'</div>';dates.forEach(function(date){html+='<div class="analytics-heatmap-row"><span>'+esc(date.slice(5).replace('-','/'))+'</span>'+sources.map(function(source){var value=values.get(date+'|'+source)||0,level=value?Math.max(.12,value/max):0;return '<i style="--heat:'+level+'" title="'+esc(date+' / '+source+' / '+value+'件')+'">'+(value||'')+'</i>';}).join('')+'</div>';});host.innerHTML=html;host.style.setProperty('--heat-columns',sources.length);}
  function analyticsCsvCell(value){var text=String(value==null?'':value);if(/^[=+\-@]/.test(text))text="'"+text;return '"'+text.replace(/"/g,'""')+'"';}
  function exportAnalyticsCsv(){var data=analyticsCurrentData,rows=data&&data.entries||[];if(!rows.length){toast('書き出せる流入データがありません');return;}var header=['date','source','referrer_domain','landing_page','landing_path','visits'],lines=[header.map(analyticsCsvCell).join(',')];rows.slice().sort(function(a,b){return a.date.localeCompare(b.date)||a.source.localeCompare(b.source,'ja')||a.request_path.localeCompare(b.request_path);}).forEach(function(row){lines.push([row.date,row.source,row.referer_host||'',row.landing_page,row.request_path,Number(row.visits||0)].map(analyticsCsvCell).join(','));});var blob=new Blob(['\ufeff'+lines.join('\r\n')],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='totonoe-traffic-'+data.filters.start+'-'+data.filters.end+'.csv';document.body.appendChild(link);link.click();link.remove();setTimeout(function(){URL.revokeObjectURL(url);},0);toast('CSVを書き出しました');}
  function analyticsChannelLabel(value){return {x:'X',instagram:'Instagram',youtube:'YouTube',email:'メール',event:'イベント',website:'Webサイト',ai:'AI',other:'その他'}[value]||value;}
  function analyticsInitiativeTypeLabel(value){return {social_post:'SNS投稿',article:'記事公開',video:'動画公開',email:'メール配信',event:'イベント',site_update:'サイト改修',advertising:'広告',other:'その他'}[value]||value;}
  function analyticsResultLabel(value){return {pending:'未振り返り',success:'成功',partial:'一部成功',improve:'改善して継続',hold:'保留'}[value]||'未振り返り';}
  function analyticsMemberName(id){var person=TOTONOE_MEMBERS.find(function(member){return member.id===id;});return person?person.name:'';}
  function analyticsFilteredInitiatives(items){var owner=document.getElementById('analyticsOwnerFilter').value;return (items||[]).filter(function(item){return owner==='all'||item.primary_owner_id===owner||(item.collaborator_ids||[]).includes(owner);});}
  function analyticsDateShift(date,days){var value=new Date(date+'T00:00:00Z');value.setUTCDate(value.getUTCDate()+days);return value.toISOString().slice(0,10);}
  function analyticsInitiativeEffect(initiative,daily){var map=new Map((daily||[]).map(function(row){return [row.date,Number(row.visits||0)];})),beforeDates=[],afterDates=[];for(var i=7;i>=1;i--)beforeDates.push(analyticsDateShift(initiative.occurred_on,-i));for(var j=0;j<7;j++)afterDates.push(analyticsDateShift(initiative.occurred_on,j));if(!beforeDates.concat(afterDates).every(function(date){return map.has(date);}))return null;var before=beforeDates.reduce(function(total,date){return total+map.get(date);},0),after=afterDates.reduce(function(total,date){return total+map.get(date);},0);return {before:before,after:after,change:after-before};}
  function renderAnalyticsInitiatives(items,daily){var list=analyticsFilteredInitiatives(items).slice().sort(function(a,b){return b.occurred_on.localeCompare(a.occurred_on)||String(b.updated_at||'').localeCompare(String(a.updated_at||''));}),host=document.getElementById('analyticsInitiatives'),canEdit=state.role!=='viewer';document.querySelector('[data-action="new-analytics-initiative"]').hidden=!canEdit;document.querySelector('[data-action="export-analytics-initiatives"]').disabled=!list.length;host.innerHTML=list.length?list.map(function(item){var effect=analyticsInitiativeEffect(item,daily),change=effect?(effect.change>0?'+':'')+effect.change.toLocaleString('ja-JP')+'件':'比較期間不足',effectHtml=effect?'<span>前7日 '+effect.before.toLocaleString('ja-JP')+' → 後7日 '+effect.after.toLocaleString('ja-JP')+'</span><strong>'+change+'</strong><small>因果関係ではなく前後差</small>':'<strong>'+change+'</strong><small>前後7日を含む期間を選択</small>',collaborators=(item.collaborator_ids||[]).map(analyticsMemberName).filter(Boolean),owner=analyticsMemberName(item.primary_owner_id)||item.owner||'担当者未設定',reflection=item.result_status&&item.result_status!=='pending'?'<p class="analytics-initiative-reflection"><b>'+esc(analyticsResultLabel(item.result_status))+'</b> '+esc(item.learning||item.next_action||'振り返り内容は未入力')+'</p>':'';return '<article class="analytics-initiative-card"><div class="analytics-initiative-date">'+esc(item.occurred_on)+'<span>'+esc(analyticsChannelLabel(item.channel))+'</span></div><div class="analytics-initiative-main"><strong>'+esc(item.title)+'</strong><p>'+esc(item.objective||item.hypothesis||analyticsInitiativeTypeLabel(item.initiative_type))+'</p><small>主担当 '+esc(owner)+(collaborators.length?' ／ 共同 '+esc(collaborators.join('、')):'')+'</small>'+reflection+'</div><p class="analytics-initiative-destination">'+esc(item.destination_path||'誘導先未設定')+'</p><div class="analytics-initiative-effect">'+effectHtml+'</div>'+(canEdit?'<div class="analytics-initiative-edit"><button type="button" class="secondary-button" data-edit-initiative="'+esc(item.id)+'">編集</button></div>':'')+'</article>';}).join(''):'<div class="analytics-empty">選択した条件に合う施策はありません。</div>';}
  function downloadAnalyticsCsv(filename,header,rows){var lines=[header.map(analyticsCsvCell).join(',')];rows.forEach(function(row){lines.push(row.map(analyticsCsvCell).join(','));});var blob=new Blob(['\ufeff'+lines.join('\r\n')],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=filename;document.body.appendChild(link);link.click();link.remove();setTimeout(function(){URL.revokeObjectURL(url);},0);}
  function exportAnalyticsInitiatives(){var data=analyticsCurrentData,list=analyticsFilteredInitiatives(data&&data.initiatives||[]);if(!list.length){toast('書き出せる施策がありません');return;}downloadAnalyticsCsv('totonoe-initiatives-'+data.filters.start+'-'+data.filters.end+'.csv',['occurred_on','title','channel','initiative_type','destination_path','objective','hypothesis','primary_owner','collaborators','result_status','review_on','learning','next_action','reference_url','notes'],list.slice().sort(function(a,b){return a.occurred_on.localeCompare(b.occurred_on);}).map(function(item){return [item.occurred_on,item.title,analyticsChannelLabel(item.channel),analyticsInitiativeTypeLabel(item.initiative_type),item.destination_path,item.objective,item.hypothesis,analyticsMemberName(item.primary_owner_id)||item.owner,(item.collaborator_ids||[]).map(analyticsMemberName).filter(Boolean).join('、'),analyticsResultLabel(item.result_status),item.review_on,item.learning,item.next_action,item.reference_url,item.notes];}));toast('施策CSVを書き出しました');}
  function syncAnalyticsMemberChips(primaryId,collaboratorIds){var collaborators=collaboratorIds||[];document.querySelectorAll('#analyticsInitiativePrimaryOwner input').forEach(function(input){input.checked=input.value===primaryId;});document.querySelectorAll('#analyticsInitiativeCollaborators input').forEach(function(input){input.checked=collaborators.includes(input.value)&&input.value!==primaryId;input.disabled=input.value===primaryId;});}
  function openAnalyticsInitiative(id){if(state.role==='viewer'){toast('閲覧者は施策を編集できません');return;}var item=id&&analyticsCurrentData&&(analyticsCurrentData.initiatives||[]).find(function(entry){return entry.id===id;}),form=document.getElementById('analyticsInitiativeForm');form.reset();document.getElementById('analyticsInitiativeId').value=item?item.id:'';document.getElementById('analyticsInitiativeDialogTitle').textContent=item?'施策を編集':'施策を記録';document.getElementById('analyticsInitiativeDate').value=item?item.occurred_on:(document.getElementById('analyticsEnd').value||analyticsDate(0));document.getElementById('analyticsInitiativeChannel').value=item?item.channel:'x';document.getElementById('analyticsInitiativeType').value=item?item.initiative_type:'social_post';document.getElementById('analyticsInitiativeTitle').value=item?item.title:'';document.getElementById('analyticsInitiativeDestination').value=item?item.destination_path:'';document.getElementById('analyticsInitiativeObjective').value=item?item.objective:'';document.getElementById('analyticsInitiativeHypothesis').value=item?item.hypothesis:'';document.getElementById('analyticsInitiativeReference').value=item?item.reference_url:'';document.getElementById('analyticsInitiativeNotes').value=item?item.notes:'';document.getElementById('analyticsInitiativeResultStatus').value=item&&item.result_status?item.result_status:'pending';document.getElementById('analyticsInitiativeReviewOn').value=item&&item.review_on?item.review_on:'';document.getElementById('analyticsInitiativeLearning').value=item&&item.learning?item.learning:'';document.getElementById('analyticsInitiativeNextAction').value=item&&item.next_action?item.next_action:'';syncAnalyticsMemberChips(item&&item.primary_owner_id?item.primary_owner_id:TOTONOE_MEMBERS[0].id,item&&item.collaborator_ids||[]);document.getElementById('analyticsInitiativeDialog').showModal();}
  function closeAnalyticsInitiative(){var dialog=document.getElementById('analyticsInitiativeDialog');if(dialog.open)dialog.close();}
  function collectAnalyticsInitiative(){var primary=document.querySelector('#analyticsInitiativePrimaryOwner input:checked'),collaborators=[...document.querySelectorAll('#analyticsInitiativeCollaborators input:checked')].map(function(input){return input.value;});return {occurred_on:document.getElementById('analyticsInitiativeDate').value,title:document.getElementById('analyticsInitiativeTitle').value.trim(),channel:document.getElementById('analyticsInitiativeChannel').value,initiative_type:document.getElementById('analyticsInitiativeType').value,destination_path:document.getElementById('analyticsInitiativeDestination').value.trim(),objective:document.getElementById('analyticsInitiativeObjective').value.trim(),hypothesis:document.getElementById('analyticsInitiativeHypothesis').value.trim(),primary_owner_id:primary?primary.value:'',collaborator_ids:collaborators,reference_url:document.getElementById('analyticsInitiativeReference').value.trim(),notes:document.getElementById('analyticsInitiativeNotes').value.trim(),result_status:document.getElementById('analyticsInitiativeResultStatus').value,review_on:document.getElementById('analyticsInitiativeReviewOn').value,learning:document.getElementById('analyticsInitiativeLearning').value.trim(),next_action:document.getElementById('analyticsInitiativeNextAction').value.trim()};}
  async function saveAnalyticsInitiative(){var button=document.getElementById('saveAnalyticsInitiative'),id=document.getElementById('analyticsInitiativeId').value,payload=collectAnalyticsInitiative();if(!payload.occurred_on||!payload.title||!payload.primary_owner_id){toast('実施日、施策名、主担当者を入力してください');return;}button.disabled=true;button.textContent='保存中…';try{if(!API_BASE){var saved=Object.assign({id:id||'local-initiative-'+Date.now(),updated_at:new Date().toISOString()},payload),list=analyticsSample.initiatives||[],index=list.findIndex(function(item){return item.id===saved.id;});if(index>-1)list[index]=saved;else list.unshift(saved);analyticsSample.initiatives=list;}else{if(!cloudReady)throw new Error('共有環境への接続を確認してください。');await api('/api/analytics-initiatives'+(id?'/'+encodeURIComponent(id):''),{method:id?'PATCH':'POST',body:JSON.stringify(payload)});}closeAnalyticsInitiative();analyticsLoaded=false;await loadAnalytics();toast(id?'施策を更新しました':'施策を記録しました');}catch(error){toast(error.message||'施策を保存できませんでした');}finally{button.disabled=false;button.textContent='保存する';}}
  function analyticsDelta(current,previous){var change=Number(current||0)-Number(previous||0),percent=Number(previous||0)?Math.round(change/Number(previous)*100):null;return {change:change,text:(change>0?'+':'')+change.toLocaleString('ja-JP')+'件'+(percent===null?'':('（'+(percent>0?'+':'')+percent+'%）'))};}
  function renderAnalytics(data){analyticsCurrentData=data;var kpis=data.kpis||{},initiatives=data.initiatives||[],compareEnabled=document.getElementById('analyticsCompare').value==='previous',previous=data.comparison&&data.comparison.kpis||{};document.getElementById('analyticsVisits').textContent=Number(kpis.visits||0).toLocaleString('ja-JP');document.getElementById('analyticsIdentified').textContent=Number(kpis.identified_visits||0).toLocaleString('ja-JP');document.getElementById('analyticsDirect').textContent=Number(kpis.direct_visits||0).toLocaleString('ja-JP');document.getElementById('analyticsAi').textContent=Number(kpis.ai_visits||0).toLocaleString('ja-JP');[['analyticsVisitsDelta','visits'],['analyticsIdentifiedDelta','identified_visits'],['analyticsDirectDelta','direct_visits'],['analyticsAiDelta','ai_visits']].forEach(function(pair){var node=document.getElementById(pair[0]),delta=analyticsDelta(kpis[pair[1]],previous[pair[1]]);node.hidden=!compareEnabled||!data.comparison;node.textContent=delta.text;node.className=delta.change>0?'up':delta.change<0?'down':'flat';});var totalDelta=analyticsDelta(kpis.visits,previous.visits),topSource=(data.sources||[]).find(function(item){return item.label!=='直接・判別不能';})||(data.sources||[])[0];document.getElementById('analyticsInsight').textContent=compareEnabled&&data.comparison?'入口訪問は前期間（'+data.comparison.start+'〜'+data.comparison.end+'）より'+totalDelta.text+'。'+(topSource?'最多の流入元は「'+topSource.label+'」で'+Number(topSource.visits||0).toLocaleString('ja-JP')+'件です。':'流入元データはありません。'):(topSource?'最多の流入元は「'+topSource.label+'」で'+Number(topSource.visits||0).toLocaleString('ja-JP')+'件です。':'期間内の流入はありません。');document.getElementById('analyticsLastUpdated').textContent=(data.preview?'確認用データ ':'最終取得 ')+new Date(data.generated_at).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'});var quality=document.getElementById('analyticsQuality');quality.textContent=(data.preview?'ローカル確認用の実測スナップショットです。':'Cloudflareから取得しました。')+' 対象期間 '+data.filters.start+'〜'+data.filters.end+' ／ Bot除外';quality.classList.remove('warning');var filtered=analyticsFilteredInitiatives(initiatives);renderAnalyticsBars('analyticsSources',data.sources);renderAnalyticsBars('analyticsLandings',data.landings);renderAnalyticsTrend(data.daily,filtered);renderAnalyticsHeatmap(data.entries,data.daily);renderAnalyticsInitiatives(initiatives,data.daily);renderAnalyticsDailySources(data.entries);renderAnalyticsJourneys(data.journeys);document.querySelector('[data-action="export-analytics-csv"]').disabled=!(data.entries||[]).length;}
  async function loadAnalytics(){if(analyticsLoading)return;analyticsLoading=true;setAnalyticsLoading('アクセス情報を読み込んでいます。');syncAnalyticsDates();try{var data;if(!API_BASE){data=analyticsSample;}else if(!cloudReady){throw new Error('共有環境への接続を確認してください。');}else{var params=new URLSearchParams({start:document.getElementById('analyticsStart').value,end:document.getElementById('analyticsEnd').value}),results=await Promise.all([api('/api/analytics-summary?'+params.toString()),api('/api/analytics-initiatives?'+params.toString())]);data=results[0];data.initiatives=results[1].initiatives||[];}renderAnalytics(data);analyticsLoaded=true;}catch(error){var message=error.data&&error.data.error==='analytics_not_configured'?'Cloudflare Analyticsの読み取り設定が必要です。':error.message||'アクセス情報を取得できませんでした。';setAnalyticsLoading(message,true);toast(message);}finally{analyticsLoading=false;}}
  function initializeAnalyticsMembers(){var primary=document.getElementById('analyticsInitiativePrimaryOwner'),collaborators=document.getElementById('analyticsInitiativeCollaborators'),filter=document.getElementById('analyticsOwnerFilter');primary.innerHTML=TOTONOE_MEMBERS.map(function(person,index){return '<label><input type="radio" name="analytics-primary-owner" value="'+esc(person.id)+'"'+(index===0?' checked':'')+'><span>'+esc(person.name)+'</span></label>';}).join('');collaborators.innerHTML=TOTONOE_MEMBERS.map(function(person){return '<label><input type="checkbox" value="'+esc(person.id)+'"><span>'+esc(person.name)+'</span></label>';}).join('');filter.insertAdjacentHTML('beforeend',TOTONOE_MEMBERS.map(function(person){return '<option value="'+esc(person.id)+'">'+esc(person.name)+'</option>';}).join(''));primary.addEventListener('change',function(){var selected=primary.querySelector('input:checked');syncAnalyticsMemberChips(selected?selected.value:'',Array.from(collaborators.querySelectorAll('input:checked')).map(function(input){return input.value;}));});}
  ...
  function renderPosts(){var list=filteredPosts();document.getElementById('postTable').innerHTML=list.length?list.map(function(p){var status=postState(p),path=esc(publicArticlePath(p)),title=p.deleted_at?'<div class="post-title-static"><strong>'+esc(p.title)+'</strong><small>'+path+'</small></div>':'<button class="post-title-button" data-edit="'+esc(p.id)+'"><strong>'+esc(p.title)+'</strong><small>'+path+'</small></button>';return '<div class="post-table-row">'+title+'<span>'+esc(normalizeType(p.content_type)==='column'?destinationLabel(p):contentTypeLabel(p.content_type))+'</span><span class="badge '+status.key+'">'+status.label+'</span><span>'+p.updated+'</span><div class="row-actions">'+articleActions(p)+'</div></div>';}).join(''):'<div class="empty-state">該当するコンテンツはありません。</div>';}
  ...
  function renderMembers(){document.getElementById('memberList').innerHTML=state.members.map(function(m,i){var key=m.id||String(i);var own=state.currentEmail&&m.email.toLowerCase()===state.currentEmail.toLowerCase();var roleControl=m.role==='admin'?'<select class="role-select" disabled aria-label="管理者権限は固定"><option>管理者（固定）</option></select>':'<select class="role-select" data-member-role="'+esc(key)+'"><option value="editor" '+(m.role==='editor'?'selected':'')+'>編集者</option><option value="viewer" '+(m.role==='viewer'?'selected':'')+'>閲覧者</option></select>';var statusControl=m.role==='admin'?'<button type="button" class="row-menu" disabled>固定</button>':'<button type="button" class="row-menu" data-member-status="'+esc(key)+'" aria-label="'+(m.status==='disabled'?'利用を再開':'利用を停止')+'">'+(m.status==='disabled'?'再開':'停止')+'</button>';return '<div class="member-row"><div class="member-identity"><span class="member-avatar">'+esc(m.initial||'M')+'</span><div><strong>'+esc(m.name||'メンバー')+(own?'（自分）':'')+'</strong><small>'+esc(m.email)+'</small></div></div>'+roleControl+'<span class="member-status">● '+memberStatusLabel(m.status)+'</span>'+statusControl+'</div>';}).join('');}
  ...
  function blankPost(template,typeOverride){if(busyPublishing)return;flushPending();clearTimeout(scheduleSave.timer);sourceSequence++;resetLinkButton();sourceValues={};selectedBubble=null;editorRange=null;state.editingId=null;pendingHero='';lastAnalyzedUrl='';els.title.value='';els.excerpt.value='';var requestedType=normalizeType(typeOverride||currentTypeFilter);els.contentType.value=requestedType!=='all'&&CONTENT_TYPES[requestedType]?requestedType:'column';els.category.value='content';els.destination.value='tsuzuri';els.mainActor.value='';setSelectedSpeakers([]);els.tags.value='';syncTopicTagSuggestions();els.mediaUrl.value='';els.episodeNo.value='';els.sourceDate.value='';els.sourceType.value='';els.sourceId.value='';els.slug.value='';els.editor.innerHTML=template&&!isSourceOnlyType(els.contentType.value)?'<p>この記事では、まず<strong>読者に伝えたい結論</strong>を置きます。</p><h2>いま起きていること</h2><p>背景や、現場で感じている課題を書きます。</p><h2>私たちが大切にしたいこと</h2><p>具体例や、読者に持ち帰ってほしい視点を書きます。</p><h2>まとめ</h2><p>次の一歩につながる言葉で締めくくります。</p>':'';setHeroPreview('');clearLinkPreview();syncDestinationFields();syncSourceFields();updateEditorMeta();showView('editor');setTimeout(function(){els.mediaUrl.focus();},50);}
  function editPost(id){if(busyPublishing)return;var target=state.posts.find(function(item){return item.id===id;});if(target&&!managedPost(target)){toast('このコンテンツはスタジオ外で管理してください');return;}if(target&&target.deleted_at){toast('ゴミ箱の記事は復元してから編集してください');return;}flushPending();clearTimeout(scheduleSave.timer);sourceSequence++;resetLinkButton();sourceValues={};selectedBubble=null;editorRange=null;var p=state.posts.find(function(item){return item.id===id;});if(!p)return;state.editingId=id;els.title.value=p.title;els.excerpt.value=p.excerpt;els.contentType.value=normalizeType(p.content_type);els.category.value=p.category;els.destination.value=normalizeDestination(p.destination,p.category);els.mainActor.value=p.main_actor_id||'';setSelectedSpeakers(p.speaker_ids||[]);els.tags.value=p.tags;syncTopicTagSuggestions();els.mediaUrl.value=p.media_url||'';els.episodeNo.value=p.episode_no||'';els.sourceDate.value=p.source_published_at||'';els.sourceType.value=p.source_type||'';els.sourceId.value=p.source_id||'';els.slug.value=p.slug;els.editor.innerHTML=safeBody(p.body);els.editor.querySelectorAll('img').forEach(function(img){img.src=previewUrl(img.getAttribute('src'));});setHeroPreview(p.hero||'');if(p.media_url){var preview=sourceFromUrl(p.media_url);preview.title=p.title;preview.description=p.excerpt;preview.image=p.hero||preview.image;preview.published_at=p.source_published_at||'';preview.episode_no=p.episode_no||null;renderLinkPreview(preview);lastAnalyzedUrl=p.media_url;}else clearLinkPreview();syncDestinationFields();syncSourceFields();document.getElementById('visibilityTitle').textContent=p.status==='published'?'公開中':'下書き';document.getElementById('visibilityCopy').textContent=p.status==='published'?'サイトに表示されています':'サイトには表示されません';updateEditorMeta();showView('editor');}
  function slugify(v){return v.toLowerCase().trim().replace(/[\s　]+/g,'-').replace(/[^a-z0-9\-]/g,'').replace(/-+/g,'-').replace(/^-|-$/g,'');}
  function collect(status){var existing=state.posts.find(function(p){return p.id===state.editingId;});var p=existing||{id:'article_'+crypto.randomUUID(),hero:'',revision:0,history:[],deleted_at:''};p.content_type=normalizeType(els.contentType.value);var weekend=p.content_type==='weekend';p.title=weekend?'次回開催':els.title.value.trim()||'無題のコンテンツ';p.excerpt=weekend?'':els.excerpt.value.trim();p.destination=p.content_type==='column'?normalizeDestination(els.destination.value,els.category.value):'tsuzuri';p.category=p.destination==='characters'?'character-story':(els.category.value==='character-story'?'content':els.category.value);p.main_actor_id=els.mainActor.value||'';p.speaker_ids=weekend?[]:selectedSpeakerIds();p.tags=weekend?'':els.tags.value.trim();p.media_url=weekend?'':els.mediaUrl.value.trim();p.episode_no=weekend?null:(els.episodeNo.value?Number(els.episodeNo.value):null);p.source_published_at=weekend?'':els.sourceDate.value||'';p.source_type=weekend?'':els.sourceType.value||sourceFromUrl(p.media_url).kind;p.source_id=weekend?'':els.sourceId.value||sourceFromUrl(p.media_url).source_id;p.slug=weekend?(existing&&existing.slug||'weekend-'+Date.now()):els.slug.value.trim()||slugify(p.title)||'content-'+Date.now();p.body=weekend?'':safeBody(els.editor.innerHTML).replace(/\/api\/(?:tsuzuri|column)-studio\/media\//g,'/column-media/');p.hero=pendingHero;p.status=status||p.status||'draft';p.updated=dateLabel();checkpoint(p);rememberTags(p.tags);if(!existing)state.posts.unshift(p);state.editingId=p.id;els.title.value=p.title;els.slug.value=p.slug;persist();renderDashboard();renderPosts();return p;}
  ...
  async function publish(){
    ...
    if(sourceOnly&&els.contentType.value!=='weekend'&&!els.mediaUrl.value.trim()){toast(els.contentType.value==='podcast'?'stand.fmのURLを入力してください':'YouTube URLを入力してください');return;}
    ...
    if(els.contentType.value!=='weekend'&&!els.title.value.trim()){toast('タイトルを入力してください');return;}
    if(els.contentType.value==='weekend'&&!pendingHero){toast('次回開催のサムネイル画像を設定してください');return;}
  ...
  function preview(){var media=safeUrl(els.mediaUrl.value.trim()),hero=safeUrl(pendingHero,true),heroBox=document.getElementById('previewHero');heroBox.innerHTML=hero?'<img src="'+esc(previewUrl(hero))+'" alt="">':'';heroBox.hidden=!hero;document.getElementById('previewCategory').textContent=contentTypeLabel(els.contentType.value).toUpperCase();document.getElementById('previewTitle').textContent=els.title.value||'無題のコンテンツ';document.getElementById('previewLead').textContent=els.excerpt.value;document.getElementById('previewMainActor').textContent=personById(els.mainActor.value).name;document.getElementById('previewSpeakers').textContent=speakerNames(selectedSpeakerIds());document.getElementById('previewMediaUrl').innerHTML=media?'<a href="'+esc(media)+'" target="_blank" rel="noopener">'+esc(externalLinkLabel(els.contentType.value,media))+' ↗</a><br><small>'+esc(media)+'</small>':'';document.getElementById('previewBody').innerHTML=safeBody(els.editor.innerHTML)||'<p>本文はまだありません。</p>';document.getElementById('previewDate').textContent=dateLabel();document.getElementById('previewDialog').showModal();}
  var textLinkRange=null,selectedTextLink=null;
  function editorLinkFrom(node){var element=node&&node.nodeType===1?node:node&&node.parentElement;var link=element&&element.closest('a');return link&&els.editor.contains(link)?link:null;}
  function closeTextLinkDialog(){var dialog=document.getElementById('textLinkDialog');if(dialog.open)dialog.close();textLinkRange=null;selectedTextLink=null;}
  function openTextLinkDialog(){
    var selection=window.getSelection(),range=selection.rangeCount&&els.editor.contains(selection.anchorNode)?selection.getRangeAt(0).cloneRange():editorRange&&editorRange.cloneRange();
    var link=selection.rangeCount?editorLinkFrom(selection.anchorNode):null;
    if(!link&&range)link=editorLinkFrom(range.commonAncestorContainer);
    if((!range||range.collapsed)&&!link){toast('リンクにしたい文字を選択してください');return;}
    selectedTextLink=link;textLinkRange=range;
    document.getElementById('textLinkSelection').textContent='選択中：'+(link?link.textContent:range.toString());
    document.getElementById('textLinkUrl').value=link?link.getAttribute('href')||'':'https://';
    document.getElementById('removeTextLink').hidden=!link;
    document.getElementById('textLinkDialog').showModal();
    setTimeout(function(){var input=document.getElementById('textLinkUrl');input.focus();if(!link)input.select();},0);
  }
  function applyTextLink(){
    var url=safeUrl(document.getElementById('textLinkUrl').value,false);
    if(!url){toast('安全なhttpまたはhttpsリンクを入力してください');return;}
    if(selectedTextLink&&els.editor.contains(selectedTextLink)){selectedTextLink.setAttribute('href',url);selectedTextLink.setAttribute('target','_blank');selectedTextLink.setAttribute('rel','noopener noreferrer');}
    else if(textLinkRange&&!textLinkRange.collapsed&&els.editor.contains(textLinkRange.commonAncestorContainer)){
      var link=document.createElement('a');link.href=url;link.target='_blank';link.rel='noopener noreferrer';
      try{textLinkRange.surroundContents(link);}catch(error){link.appendChild(textLinkRange.extractContents());textLinkRange.insertNode(link);}
    }else{toast('選択範囲を確認できませんでした');return;}
    closeTextLinkDialog();scheduleSave();toast('テキストにリンクを設定しました');
  }
  function removeTextLink(){
    var link=selectedTextLink;if(!link||!els.editor.contains(link)){toast('解除するリンク内にカーソルを置いてください');return;}
    link.replaceWith.apply(link,Array.from(link.childNodes));closeTextLinkDialog();scheduleSave();toast('リンクを解除しました');
  }
  function format(command){if(command==='link'){openTextLinkDialog();return;}els.editor.focus();if(command==='marker'){document.execCommand('hiliteColor',false,document.getElementById('markerColor').value);}else if(command==='blockquote'){document.execCommand('formatBlock',false,'blockquote');}else{document.execCommand(command,false,null);}scheduleSave();}
  function insertMemberBoundary(){
    var existing=els.editor.querySelector('.member-content-boundary');
    if(existing){
      existing.scrollIntoView({behavior:'smooth',block:'center'});
      if(confirm('会員限定ラインは1記事に1本だけ設定できます。現在のラインを削除しますか？')){existing.remove();scheduleSave();toast('会員限定ラインを削除しました');}
      return;
    }
    ...
    var bubble=node&&node.closest&&node.closest('.editor-bubble');if(bubble){range.setStartAfter(bubble);range.collapse(true);}
    selection.removeAllRanges();selection.addRange(range);
    document.execCommand('insertHTML',false,'<div class="member-content-boundary" contenteditable="false"><strong>ここから会員限定</strong><span>この下はログイン中の会員だけに表示されます</span></div><p><br></p>');
    scheduleSave();toast('会員限定ラインを追加しました');
  }
  ...
  document.getElementById('blockFormat').addEventListener('change',function(){els.editor.focus();document.execCommand('formatBlock',false,this.value);updateEditorMeta();scheduleSave();});
  [els.editor,els.title,els.excerpt,els.contentType,els.category,els.destination,els.tags,els.slug,els.mainActor,els.speakers,els.mediaUrl,els.episodeNo,els.sourceDate].forEach(function(el){el.addEventListener('input',function(){updateEditorMeta();scheduleSave();});el.addEventListener('change',function(){updateEditorMeta();scheduleSave();});});
  els.contentType.addEventListener('change',function(){syncDestinationFields();syncSourceFields();});
  els.destination.addEventListener('change',syncDestinationFields);
  ...
  ['billingMonth','billingRange','billingProduct'].forEach(function(id){document.getElementById(id).addEventListener('change',function(){billingLoaded=false;loadBilling();});});
  document.getElementById('analyticsRange').addEventListener('change',function(){syncAnalyticsDates();analyticsLoaded=false;loadAnalytics();});
  document.getElementById('analyticsCompare').addEventListener('change',function(){if(analyticsCurrentData)renderAnalytics(analyticsCurrentData);});
  document.getElementById('analyticsOwnerFilter').addEventListener('change',function(){if(analyticsCurrentData)renderAnalytics(analyticsCurrentData);});
  ['analyticsStart','analyticsEnd'].forEach(function(id){document.getElementById(id).addEventListener('change',function(){if(document.getElementById('analyticsRange').value==='custom'){analyticsLoaded=false;loadAnalytics();}});});
  document.getElementById('analyticsInitiativeForm').addEventListener('submit',function(event){event.preventDefault();saveAnalyticsInitiative();});
  ...
  document.getElementById('billingMonth').value=japanMonth();initializeAnalyticsMembers();syncAnalyticsDates();
  renderTopicTagSuggestions();syncDestinationFields();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();var initialView=location.hash.replace('#','');if(['tsuzuri','column','podcast','video','archive','weekend'].includes(initialView))openContentView(initialView);else if(initialView==='analytics')showView('analytics');bootRemote();
})();
