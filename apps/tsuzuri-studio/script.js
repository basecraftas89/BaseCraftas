(function(){
  ...
  function publicArticlePath(post){var type=normalizeType(post.content_type);if(type==='weekend')return '/weekend-ai.html#schedule';var directory=type==='column'?'tsuzuri':(['video','learning'].includes(type)?'tsumami':'contents');return '/'+directory+'/'+post.slug+'.html';}
...
state.members=[memberFromApi(me.member)];
  ...
  renderTopicTagSuggestions();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();var initialView=location.hash.replace('#','');if(['tsuzuri','column','podcast','video','archive','weekend'].includes(initialView))openContentView(initialView);bootRemote();
})();
