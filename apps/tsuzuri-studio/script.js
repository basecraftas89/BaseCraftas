...
state.members=[memberFromApi(me.member)];
  ...
  renderTopicTagSuggestions();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();renderQualifications();var initialView=location.hash.replace('#','');if(['tsuzuri','column','podcast','video','archive','weekend'].includes(initialView))openContentView(initialView);bootRemote();
})();
