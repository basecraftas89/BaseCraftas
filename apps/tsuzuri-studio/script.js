(function(){
  ...
  function syncEditorMode(){
    ...
    document.getElementById('episodeNoField').classList.toggle('source-only-hidden',type==='weekend');
    document.getElementById('sourceDateLabel').textContent=type==='weekend'?'開催日':'開催日・公開日';
...
state.members=[memberFromApi(me.member)];
  ...
  async function publish(){
    ...
    if(els.contentType.value==='weekend'&&!els.sourceDate.value){toast('開催日を入力してください');return;}
    if(els.contentType.value==='weekend'&&els.sourceDate.value<new Date(Date.now()+9*60*60*1000).toISOString().slice(0,10)){toast('本日以降の開催日を入力してください');return;}
