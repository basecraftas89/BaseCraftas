(function(){
  ...
  var API_BASE=window.COLUMN_STUDIO_API_BASE||(location.hostname==='basecraftas.com'?'/api/column-studio':'');
  var now=new Date();
  ...
  var CONTENT_TYPES={column:'コラム',podcast:'Podcast',video:'動画',archive:'アーカイブ',seminar:'セミナー',learning:'学習',weekend:'週末AI'};
  var CONTENT_TARGETS={column:'コラム',podcast:'Podcast',video:'学習コンテンツ',archive:'アーカイブ動画',seminar:'セミナー',learning:'学習コンテンツ',weekend:'コラム'};
  ...
  var defaults={
    posts:[
      {id:'p1',title:'専門性を、届く言葉へ整える。',excerpt:'現場にある価値を、伝わる言葉へ変えていくための考え方。',category:'ブランド',destination:'totonoe',content_type:'column',tags:'ブランド, 言語化',slug:'turn-expertise-into-words',status:'published',updated:'2026.09.05',body:sampleBody,hero:'',main_actor_id:'shindo-toshiki',speaker_ids:[],media_url:''},
      {id:'p2',title:'AI導入の前に整えておきたい、3つのこと',excerpt:'ツール選びより先に、現場で共有しておきたい視点を整理します。',category:'AI・業務改善',destination:'totonoe',content_type:'column',tags:'AI, 業務改善',slug:'before-ai-adoption',status:'draft',updated:'2026.09.04',body:'<p>AIを導入するとき、最初に考えるべきことはツールの名前ではありません。</p><h2>目的をひとつに絞る</h2><p><mark>誰の、どんな時間を取り戻したいか</mark>を先に決めると、必要な仕組みが見えやすくなります。</p>',hero:'',main_actor_id:'shindo-toshiki',speaker_ids:[],media_url:''},
      {id:'p3',title:'Podcastで深める、現場の問い',excerpt:'音声コンテンツを記事としても残していくための下書きです。',category:'AI・業務改善',destination:'totonoe',content_type:'podcast',tags:'Podcast, 対話',slug:'podcast-field-questions',status:'draft',updated:'2026.09.01',body:'<p>PodcastのURLを設定すると、公開ページから元コンテンツへ案内できます。</p>',hero:'',main_actor_id:'shindo-toshiki',speaker_ids:['kajiwara-yusuke'],media_url:''}
    ],
    members:[
  ...
  function matchesContentView(post,type){var actual=normalizeType(post.content_type);if(type==='all')return true;if(type==='column')return actual==='column'||actual==='weekend';if(type==='video')return actual==='video'||actual==='learning';return actual===type;}
  ...
  function sourceFromUrl(value){
    var result={url:String(value||'').trim(),kind:'external',provider:'外部リンク',recommended_content_type:'column',public_target:'コラム',source_id:'',title:'',description:'',image:'',published_at:'',episode_no:null};
    try{
    ...
    result.public_target=CONTENT_TARGETS[result.recommended_content_type]||'コラム';return result;
  }
  ...
  function persist(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));els.lastSaved.textContent=new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'});return true;}catch(error){els.saveState.textContent='端末への保存に失敗しました。容量を確認してください';return false;}}
  ...
  function api(path,options){return fetch(API_BASE+path,Object.assign({credentials:'include',headers:{'content-type':'application/json'}},options||{})).then(function(res){return res.json().then(function(data){if(!res.ok){var error=new Error(data.message||data.error||'api_error');error.status=res.status;error.data=data;throw error;}return data;});});}
  ...
  function uploadHero(file){var fd=new FormData();fd.append('file',file);var p=state.posts.find(function(item){return item.id===state.editingId;});if(p&&p.revision)fd.append('article_id',p.id);return fetch(API_BASE+'/api/assets',{method:'POST',credentials:'include',body:fd}).then(function(res){return res.json().then(function(data){if(!res.ok)throw new Error(data.error||'画像を保存できませんでした');return data.asset;});});}
  function isSourceOnlyType(type){return ['podcast','video','learning'].includes(normalizeType(type));}
  function syncEditorMode(){
    ...
    var copy=type==='podcast'?['stand.fmのURLを貼り付け','stand.fmのリンクからタイトルと概要を読み込みます。','https://stand.fm/episodes/...','Podcastのカテゴリタグ']:['YouTube URLを貼り付け','YouTubeのリンクからタイトルとサムネイルを読み込みます。','https://www.youtube.com/watch?v=...','動画のカテゴリタグ'];
    if(sourceOnly){title.textContent=copy[0];help.textContent=copy[1];els.mediaUrl.placeholder=copy[2];document.getElementById('linkStatus').textContent=type==='podcast'?'stand.fmのURLを貼り付けて「情報を取得」を押してください。':'YouTube URLを貼り付けて「情報を取得」を押してください。';tagLabel.textContent=copy[3];if(!els.mainActor.value)els.mainActor.value='shindo-toshiki';}
    else{title.textContent='元コンテンツのURLを貼り付け';help.textContent='note・stand.fm・YouTube・Google Drive・セミナーURLを自動判定します。';els.mediaUrl.placeholder='https://note.com/... / https://stand.fm/...';tagLabel.textContent='Topic Tag';}
    ...
    document.getElementById('sourceDetailFields').classList.toggle('source-only-hidden',sourceOnly);
    document.getElementById('imageDrop').closest('.setting-block').classList.toggle('source-only-hidden',sourceOnly);
  ...
  function setHeroPreview(url){var drop=document.getElementById('imageDrop');pendingHero=url||'';drop.style.backgroundImage=url?'url("'+String(url).replace(/"/g,'%22')+'")':'';drop.classList.toggle('has-image',Boolean(url));}
  ...
  function renderLinkPreview(preview){var card=document.getElementById('linkPreviewCard');var status=document.getElementById('linkStatus');var image=document.getElementById('linkPreviewImage');card.hidden=false;image.style.backgroundImage=preview.image?'url("'+String(preview.image).replace(/"/g,'%22')+'")':'';document.getElementById('linkPreviewProvider').textContent=preview.provider||preview.kind||'外部リンク';document.getElementById('linkPreviewTarget').textContent='反映先: '+(preview.public_target||CONTENT_TARGETS[preview.recommended_content_type]||'コラム');document.getElementById('linkPreviewTitle').textContent=preview.title||'タイトルは手入力してください';document.getElementById('linkPreviewDescription').textContent=preview.description||'取得できない項目はそのまま編集できます。';status.className='link-status success';status.textContent='情報を取得しました。内容を確認してから公開してください。';}
  function applyLinkPreview(preview){
    els.contentType.value=normalizeType(preview.recommended_content_type);els.sourceType.value=preview.kind||'external';els.sourceId.value=preview.source_id||'';
  ...
  async function bootRemote(){
    ...
    try{
      ...
      state.role=me.member.role;state.currentEmail=me.email||me.member.email||'';if(state.role!=='admin')state.members=[memberFromApi(me.member)];cloudReady=true;
      var results=await Promise.all([api('/api/articles'),api('/api/tags'),state.role==='admin'?api('/api/members'):Promise.resolve(null),api('/api/archive-candidates')]);
      ...
      if(results[2])state.members=(results[2].members||[]).map(memberFromApi);
      state.archiveCandidates=results[3].candidates||[];state.archiveSync=results[3].sync||null;
      persist();applyRole(state.role);renderDashboard();renderPosts();renderMembers();renderArchiveCandidates();
      els.saveState.textContent=local.length?'端末に未共有の下書きがあります':'チームに接続済み';
    }catch(error){cloudReady=false;els.saveState.textContent='共有未接続・端末保存のみ';toast(error.message);}
  }
  ...
  function openContentView(type){currentTypeFilter=type||'all';setStatusFilter('all');var labels={all:['すべてのコンテンツ','コラム・Podcast・動画・アーカイブをまとめて管理します。'],column:['コラム','記事・note連携・週末のAI整え習慣を管理します。'],podcast:['Podcast','音声エピソードと公開URLを管理します。'],video:['動画','動画・学習コンテンツと公開URLを管理します。'],archive:['アーカイブ','Google Driveの新着を確認し、下書きから公開します。']};var copy=labels[currentTypeFilter]||labels.all;document.getElementById('contentListTitle').textContent=copy[0];document.getElementById('contentListCopy').textContent=copy[1];var panel=document.getElementById('archiveSyncPanel');if(panel)panel.hidden=currentTypeFilter!=='archive';if(currentTypeFilter==='archive')renderArchiveCandidates();if(history.replaceState){if(currentTypeFilter==='all')history.replaceState(null,'',location.pathname+location.search);else history.replaceState(null,'','#'+currentTypeFilter);}showView('posts');}
  function renderDashboard(){var active=state.posts.filter(function(p){return !p.deleted_at;});document.getElementById('publishedCount').textContent=active.filter(function(p){return p.status==='published';}).length;document.getElementById('draftCount').textContent=active.filter(function(p){return p.status!=='published';}).length;document.getElementById('monthCount').textContent=active.filter(function(p){return p.updated.slice(0,7)===dateLabel().slice(0,7);}).length;document.getElementById('memberCount').textContent=state.members.length;document.getElementById('postNavCount').textContent=active.length;document.getElementById('columnNavCount').textContent=active.filter(function(p){return matchesContentView(p,'column');}).length;document.getElementById('podcastNavCount').textContent=active.filter(function(p){return matchesContentView(p,'podcast');}).length;document.getElementById('videoNavCount').textContent=active.filter(function(p){return matchesContentView(p,'video');}).length;document.getElementById('archiveNavCount').textContent=active.filter(function(p){return matchesContentView(p,'archive');}).length;document.getElementById('recentPosts').innerHTML=active.slice().sort(function(a,b){return b.updated.localeCompare(a.updated);}).slice(0,4).map(function(p){var status=postState(p);return '<div class="recent-row"><button class="post-title-button" data-edit="'+p.id+'"><strong>'+esc(p.title)+'</strong><small>'+esc(contentTypeLabel(p.content_type))+' ・ '+esc(personById(p.main_actor_id).name)+'</small></button><span class="badge '+status.key+'">'+status.label+'</span><span class="row-date">'+p.updated+'</span></div>';}).join('');}
  ...
  function renderPosts(){var list=filteredPosts();document.getElementById('postTable').innerHTML=list.length?list.map(function(p){var status=postState(p),title=p.deleted_at?'<div class="post-title-static"><strong>'+esc(p.title)+'</strong><small>/contents/'+esc(p.slug)+'.html</small></div>':'<button class="post-title-button" data-edit="'+esc(p.id)+'"><strong>'+esc(p.title)+'</strong><small>/contents/'+esc(p.slug)+'.html</small></button>';return '<div class="post-table-row">'+title+'<span>'+esc(contentTypeLabel(p.content_type))+'</span><span class="badge '+status.key+'">'+status.label+'</span><span>'+p.updated+'</span><div class="row-actions">'+articleActions(p)+'</div></div>';}).join(''):'<div class="empty-state">該当するコンテンツはありません。</div>';}
  ...
  function formatArchiveTime(value){if(!value)return '未確認';var date=new Date(String(value).replace(' ','T')+(String(value).includes('Z')?'':'Z'));return Number.isNaN(date.getTime())?value:date.toLocaleString('ja-JP',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});}
  function renderArchiveCandidates(){
    var panel=document.getElementById('archiveSyncPanel'),status=document.getElementById('archiveSyncStatus'),list=document.getElementById('archiveCandidateList');if(!panel||!status||!list)return;
    panel.hidden=currentTypeFilter!=='archive';if(panel.hidden)return;
    if(!API_BASE){status.textContent='端末版では画面を確認できます。Driveの取得は本番ダッシュボードで実行します。';status.className='archive-sync-status';list.innerHTML='<div class="archive-empty">新着候補は本番接続後に表示されます。</div>';return;}
    var sync=state.archiveSync||{};status.textContent=sync.last_error?'前回の確認でエラー：'+sync.last_error:'最終確認：'+formatArchiveTime(sync.last_checked_at)+' ／ 前回の新着 '+Number(sync.new_count||0)+'件';status.className='archive-sync-status'+(sync.last_error?' error':'');
    var items=state.archiveCandidates||[];list.innerHTML=items.length?items.map(function(item){var imported=item.status==='imported';return '<article class="archive-candidate '+(imported?'imported':'')+'"><div><strong>'+esc(item.name)+'</strong><small>'+esc(item.mime_type||'メディア')+' ・ '+esc(String(item.created_time||'').slice(0,10)||'日付未設定')+'</small></div><div class="archive-candidate-actions"><a href="'+esc(item.web_view_link)+'" target="_blank" rel="noopener">Driveで確認 ↗</a>'+(imported?'<span class="badge published">下書き作成済み</span>':'<button class="primary-button" type="button" data-import-archive="'+esc(item.id)+'">下書きに取り込む</button>')+'</div></article>';}).join(''):'<div class="archive-empty">新しいアーカイブ候補はありません。</div>';
  }
  async function loadArchiveCandidates(){if(!API_BASE)return;var result=await api('/api/archive-candidates');state.archiveCandidates=result.candidates||[];state.archiveSync=result.sync||null;persist();renderArchiveCandidates();}
  async function scanArchives(){
    if(!API_BASE){toast('自動確認は本番ダッシュボードで利用できます');return;}
    var button=document.querySelector('[data-action="scan-archives"]');button.disabled=true;button.textContent='確認中…';
    try{var result=await api('/api/archive-candidates/scan',{method:'POST',body:'{}'});await loadArchiveCandidates();toast('Driveを確認しました。新着 '+Number(result.new_count||0)+'件です');}
    catch(error){await loadArchiveCandidates().catch(function(){});toast(error.message||'Driveを確認できませんでした');}
    finally{button.disabled=false;button.textContent='今すぐ確認';}
  }
  async function importArchive(id){
    if(!API_BASE){toast('下書きへの取り込みは本番ダッシュボードで利用できます');return;}
    try{var result=await api('/api/archive-candidates/'+encodeURIComponent(id)+'/import',{method:'POST',body:'{}'});var article=articleFromApi(result.article);var index=state.posts.findIndex(function(post){return post.id===article.id;});if(index>-1)state.posts[index]=article;else state.posts.unshift(article);await loadArchiveCandidates();persist();renderDashboard();editPost(article.id);toast('アーカイブを下書きに取り込みました。Main Actorを選んで確認してください');}
    catch(error){toast(error.message||'下書きへ取り込めませんでした');}
  }
  ...
  async function publish(){
    ...
    if(sourceOnly&&['video','learning'].includes(els.contentType.value)&&profile.kind!=='video'){toast('YouTube URLを入力してください');return;}
    if(sourceOnly&&!els.title.value.trim())els.title.value=els.contentType.value==='podcast'?'Podcastエピソード':'学習動画';
  ...
  renderTopicTagSuggestions();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();var initialView=location.hash.replace('#','');if(['column','podcast','video','archive'].includes(initialView))openContentView(initialView);bootRemote();
})();
