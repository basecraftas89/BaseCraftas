(function(){
  ...
  function syncSourceFields(){var type=normalizeType(els.contentType.value);document.getElementById('sourceDetailFields').hidden=['podcast','archive','seminar'].indexOf(type)===-1;}
  ...
  async function bootRemote(){
    ...
    try{
      ...
      var results=await Promise.all([api('/api/articles'),api('/api/tags'),state.role==='admin'?api('/api/members'):Promise.resolve(null)]);
  ...
  function showView(name){if(busyPublishing)return;flushPending();if(name==='editor'&&state.role==='viewer'){toast('閲覧者は記事を編集できません');return;}if((name==='members'||name==='settings')&&state.role!=='admin'){toast('管理者だけが開ける画面です');return;}document.querySelectorAll('.view').forEach(function(v){v.classList.toggle('active',v.id==='view-'+name);});document.querySelectorAll('.nav-item').forEach(function(b){b.classList.toggle('active',b.dataset.view===name);});var active=document.getElementById('view-'+name);document.getElementById('breadcrumb').innerHTML='<span>Column Studio</span><b>/</b> '+active.dataset.title;if(name==='posts')renderPosts();if(name==='dashboard')renderDashboard();if(name==='members')renderMembers();document.getElementById('sidebar').classList.remove('open');window.scrollTo(0,0);}
  function renderDashboard(){document.getElementById('publishedCount').textContent=state.posts.filter(function(p){return p.status==='published';}).length;document.getElementById('draftCount').textContent=state.posts.filter(function(p){return p.status==='draft';}).length;document.getElementById('monthCount').textContent=state.posts.filter(function(p){return p.updated.slice(0,7)===dateLabel().slice(0,7);}).length;document.getElementById('memberCount').textContent=state.members.length;document.getElementById('postNavCount').textContent=state.posts.length;document.getElementById('recentPosts').innerHTML=state.posts.slice().sort(function(a,b){return b.updated.localeCompare(a.updated);}).slice(0,4).map(function(p){return '<div class="recent-row"><button class="post-title-button" data-edit="'+p.id+'"><strong>'+esc(p.title)+'</strong><small>'+esc(contentTypeLabel(p.content_type))+' ・ '+esc(personById(p.main_actor_id).name)+'</small></button><span class="badge '+p.status+'">'+(p.status==='published'?'公開中':'下書き')+'</span><span class="row-date">'+p.updated+'</span></div>';}).join('');}
  function filteredPosts(){var q=(document.getElementById('postSearch').value||'').toLowerCase();return state.posts.filter(function(p){var type=normalizeType(p.content_type);return(currentFilter==='all'||p.status===currentFilter)&&(currentTypeFilter==='all'||type===currentTypeFilter)&&(!q||(p.title+' '+p.tags+' '+p.category+' '+contentTypeLabel(type)+' '+speakerNames(p.speaker_ids)).toLowerCase().indexOf(q)>-1);});}
  function renderPosts(){var list=filteredPosts();document.getElementById('postTable').innerHTML=list.length?list.map(function(p){return '<div class="post-table-row"><button class="post-title-button" data-edit="'+p.id+'"><strong>'+esc(p.title)+'</strong><small>/contents/'+esc(p.slug)+'.html</small></button><span>'+esc(contentTypeLabel(p.content_type))+'</span><span class="badge '+p.status+'">'+(p.status==='published'?'公開中':'下書き')+'</span><span>'+p.updated+'</span><button class="row-menu" data-more="'+p.id+'" aria-label="その他">⋯</button></div>';}).join(''):'<div class="empty-state">該当するコンテンツはありません。</div>';}
  ...
  renderTopicTagSuggestions();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();bootRemote();
})();
