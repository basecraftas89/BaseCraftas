(function(){
  'use strict';
  if(!window.ColumnSecurity)throw new Error('安全な本文表示を読み込めません。再読み込みしてください。');
  var safeBody=window.ColumnSecurity.body,safeUrl=window.ColumnSecurity.url,previewUrl=window.ColumnSecurity.previewUrl;
  ...
  function logout(){
    ...
    clearSessionView();localStorage.removeItem(STORAGE_KEY);
  ...
  async function uploadHero(file){var current=state.posts.find(function(item){return item.id===state.editingId;});if(!current)throw new Error('先に下書きを作成してください');if(!current.revision)await syncRemoteArticle(current);var fd=new FormData();fd.append('file',file);var p=state.posts.find(function(item){return item.id===state.editingId;});if(p&&p.revision)fd.append('article_id',p.id);return fetch(API_BASE+'/api/assets',{method:'POST',credentials:'include',body:fd}).then(function(res){return res.json().then(function(data){if(!res.ok)throw new Error(data.error||'画像を保存できませんでした');return data.asset;});});}
  ...
  async function bootRemote(){
    ...
    try{
      ...
      sessionKey='column-studio-session:'+state.currentEmail.toLowerCase();
...
if(state.role!=='admin')state.members=[memberFromApi(me.member)];cloudReady=true;
