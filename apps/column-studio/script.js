...
if(state.role!=='admin')state.members=[memberFromApi(me.member)];cloudReady=true;
      var results=await Promise.all([api('/api/articles'),api('/api/tags'),state.role==='admin'?api('/api/members'):Promise.resolve(null),api('/api/archive-candidates'),state.role==='admin'?api('/api/admin/qualifications'):Promise.resolve(null)]);
      ...
      state.qualifications=results[4]?(results[4].submissions||[]):[];
      persist();applyRole(state.role);renderDashboard();renderPosts();renderMembers();renderArchiveCandidates();renderQualifications();
  ...
  async function importArchive(id){
    ...
    try{var result=await api('/api/archive-candidates/'+encodeURIComponent(id)+'/import',{method:'POST',body:'{}'});var article=articleFromApi(result.article);var index=state.posts.findIndex(function(post){return post.id===article.id;});if(index>-1)state.posts[index]=article;else state.posts.unshift(article);await loadArchiveCandidates();persist();renderDashboard();editPost(article.id);toast('アーカイブを下書きに取り込みました。回数・開催日・タイトルを確認してください');}
  ...
  renderTopicTagSuggestions();syncSourceFields();applyRole(state.role);renderDashboard();renderPosts();renderMembers();renderQualifications();var initialView=location.hash.replace('#','');if(['column','podcast','video','archive'].includes(initialView))openContentView(initialView);bootRemote();
})();
