(() => {
  ...
  function setupPricing() {
    ...
    function update() {
      const entry = config.entryFees.first[audience];
      const recurring = config.plans.curriculum[billing];
      const audienceLabel = audience === "therapist" ? "セラピスト・初回登録" : "一般・初回登録";
      const billingLabel = billing === "annual" ? "年額" : "月額";
      document.querySelector("#priceAudienceLabel").textContent = audienceLabel;
      document.querySelector("#priceTotal").textContent = yen.format(entry + recurring);
      document.querySelector("#priceBreakdown").textContent = `入会金${yen.format(entry)}円＋${billingLabel}${yen.format(recurring)}円`;
    ...
    document.querySelector("[data-preview-action]")?.addEventListener("click", () => {
      ...
      document.querySelector("#checkoutSummary").textContent = `${type}・${period}の申込内容を表示しています。`;
      dialog?.showModal();
    });
    update();
  }
