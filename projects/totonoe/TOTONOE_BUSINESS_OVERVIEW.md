...
## 2026年9月22日：つまみの固定動画廃止とStudio登録の安定化（本番反映済み）

- `service-content.js` に固定していた旧YouTube動画3件を削除し、つまみの動画表示元をStudioの公開JSONへ一本化した。GitHub `main` の `a846fcb` と静的Workerへ反映し、公開ページと公開JSONで旧3件が含まれないことを確認した。確認時のWorker Version IDは `bdfa3fc1-d34b-4ef7-8a92-95d7226c7efd`。
- YouTubeリンクの入力直後に自動保存された「無題のコンテンツ」が、情報取得後も正式タイトルへ置き換わらない問題を修正。手入力済みタイトルは再取得で上書きしない。この修正は本番Studioへ反映済み。
- 新しいYouTube動画3件（`BMhHEeAj6-I`、`0oVl1m7wBbk`、`ICu95mMyCFg`）は、タイトル・紹介文・タグを付け、管理者セッションのStudioからつまみへ公開した。YouTubeリンクの登録から共有保存、管理者公開、GitHubの公開データ更新までをダッシュボード上で実機確認済み。
- 公開中のつまみは、新3件とClaude Academy動画（`rAnUjlcZhW8`）の計4件。匿名公開ページで全4件のカードと公開JSONを確認し、旧固定動画3件が含まれないことも再確認した。確認時の静的Worker Version IDは `09df6232-68a8-4475-8a1f-3fb1b249e590`。

## 2026年9月22日：Studioのつまみ専用登録導線と専用ページ画像の統一（本番反映済み）

- Studioホームとつまみ管理画面に「YouTube動画をつまみに追加」の専用導線を設け、YouTubeリンクの貼付、情報取得、サムネイル確認、つまみへの公開までを同じ画面で行う。動画ファイルそのものをR2へ保存する方式ではなく、YouTubeを配信元として登録・公開する既存方針を維持する。
- つまみ管理画面では新規作成時の種別を動画に固定し、ボタン・説明を「YouTube動画を追加」「つまみに公開する」に切り替える。公開先は引き続き `tsumami/?video=YouTube動画ID` で、個別コラムHTMLは生成しない。
- つづり・つまみの専用ページのヒーローを、TOP・コンテンツページで採用済みの横長16:9 WebPへ統一する。コンテンツ、料金、公開範囲、事業導線は変更しない。
- GitHub `main` とCloudflare静的Workerへ反映し、Worker Version ID `357508d8-d256-4519-9d64-7dca0bdb64e0` で公開。匿名ブラウザで両ページの横長画像表示、画像HTTP 200、StudioのAccess保護継続を確認した。
...
## 2026年9月22日：TAYORIのセミナー受講特典（本番反映済み）

- 重複していたABOUTセクションを削除し、ヒーロー → 5特典スライド → 料金 → FAQの順に整理して本番公開した。
- スライド直下の重複するHTML補足を削除。画像内のセミナー受講説明は補足文と同程度の文字サイズに調整（v3）。
- ユーザー確認により、TAYORI会員は「運営が指定した対象の有料セミナーに追加料金なしで参加できる」と確定。全有料セミナー、受講回数無制限、アーカイブ視聴込み等の条件は追加していない。
- 「TAYORIで得られること」の図解を、上段に既存4項目、下段に「参加する｜有料セミナー受講」の横長枠を置く5特典構成に更新。LPの導入文・個人プラン特典・FAQにも反映。料金、受付状態、権限実装は変更していない。
- GitHub `main` とCloudflare静的Workerへ反映し、Worker Version ID `08ea0ab6-0bab-49d5-b9e2-557725996471` で公開。匿名ブラウザで文言とレイアウト、配信画像2点のHTTP 200を確認した。

## 2026年9月22日：たよりのヒーロー画像（本番反映済み）

- ツグモが原稿を整え、完成した新聞と配達かばんを用意する16:9画像を制作。たよりページの冒頭を説明文と画像の2列、900px以下では縦並びに変更した。
- 配信用WebPは1600×900px（約138KB）と800×450px（約55KB）。見出し・説明・既存の3導線はHTMLで保持し、料金・提供条件・申込準備中の状態は変更していない。本番公開済み。
...
| つまみ｜TSUMAMI | AIの要点を短い動画・PDF・スライドで提供 | 無料集客、試用価値、他サービスへの送客 | 公開中。固定動画3件を廃止し、Studio公開JSONへ一本化済み。Studioから公開した新YouTube動画3件とClaude Academy動画の計4件を公開 |
...
- `toshiki.kanto.workspace@gmail.com`、`kansai89414@gmail.com`、`base.craftas478@gmail.com` の3アカウントを固定管理者として扱い、それ以外のアカウントを管理者へ変更できない構成に更新。API Worker `b5bd9920-d53a-4b59-97d2-b98aef16e755` とD1へ本番反映し、3件だけが有効な固定管理者として表示されることを確認済み。
