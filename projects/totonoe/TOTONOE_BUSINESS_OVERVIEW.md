...
## 2026年9月22日：つまみの固定動画廃止とStudio登録の安定化（ローカル実装・未公開）

- `service-content.js` に固定していた旧YouTube動画3件を削除し、つまみの動画表示元をStudioの公開JSONへ一本化した。
- YouTubeリンクの入力直後に自動保存された「無題のコンテンツ」が、情報取得後も正式タイトルへ置き換わらない問題を修正。手入力済みタイトルは再取得で上書きしない。
- 新しいYouTube動画3件の登録・公開、旧固定動画の本番非表示、公開ページの確認は未実施。
...
| つまみ｜TSUMAMI | AIの要点を短い動画・PDF・スライドで提供 | 無料集客、試用価値、他サービスへの送客 | 公開中。固定動画3件の廃止とStudio公開JSONへの一本化をローカル実装済み・本番未反映 |
