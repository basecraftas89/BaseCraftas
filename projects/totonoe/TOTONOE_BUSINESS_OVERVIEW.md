...
## 2026年9月22日：TAYORIのセミナー受講特典（確定・ローカル反映／未公開）

- 重複していたABOUTセクションを削除し、ヒーロー → 5特典スライド → 料金 → FAQの順に整理。ローカル反映のみ。
- スライド直下の重複するHTML補足を削除。画像内のセミナー受講説明は補足文と同程度の文字サイズに調整（v3）。提供条件は変更なし・未公開。
- ユーザー確認により、TAYORI会員は「運営が指定した対象の有料セミナーに追加料金なしで参加できる」と確定。全有料セミナー、受講回数無制限、アーカイブ視聴込み等の条件は追加していない。
- 「TAYORIで得られること」の図解を、上段に既存4項目、下段に「参加する｜有料セミナー受講」の横長枠を置く5特典構成に更新。LPの導入文・個人プラン特典・FAQにも反映。料金、受付状態、権限実装は変更なし。本番未反映。

## 2026年9月22日：たよりのヒーロー画像（ローカル実装・未公開）

- ツグモが原稿を整え、完成した新聞と配達かばんを用意する16:9画像を制作。たよりページの冒頭を説明文と画像の2列、900px以下では縦並びに変更した。
- 配信用WebPは1600×900px（約138KB）と800×450px（約55KB）。見出し・説明・既存の3導線はHTMLで保持し、料金・提供条件・申込準備中の状態は変更していない。本変更は本番へ未反映。
