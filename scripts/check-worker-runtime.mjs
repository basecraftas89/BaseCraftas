...
const mf=new Miniflare(convertV4MiniflareOptions({modules:true,script:readFileSync(process.env.WORKER_TEST_PATH||'/private/tmp/column-studio-security-worker/worker.js','utf8'),compatibilityDate:'2026-09-06',compatibilityFlags:['nodejs_compat'],bindings:{ALLOW_DEV_AUTH:'true'},d1Databases:['DB'],r2Buckets:['MEDIA']}));
try{
 const db=await mf.getD1Database('DB');
 const schema=readFileSync('apps/column-studio-api/schema.sql','utf8').replace(/^--.*$/gm,'');
